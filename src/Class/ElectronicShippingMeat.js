...
/**
 * SDK for AFIP Electronic Shipping (wsremcarne)
 *
...
module.exports = class ElectronicShippingMeat extends AfipWebService {
  ...
  /**
   * Asks to AFIP Servers for receivers shipping receipts {@see WS
   * Specification item 2.5.14}
   *
   * @return array All shipping receipts
   **/
  async getShippingReceivers(statusType) {
    const res = await this.executeRequest('consultarRemitosReceptor', {
      'estadoRecepcion': statusType
    }, 'consultarRemitos');
    return res.arrayRemitos.remitosConsulta;
  }
